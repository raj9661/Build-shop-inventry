    ...
    try {
      ...
      if (res.ok) {
        ...
        setProducts(allProducts.filter((p: any) => {
          ...
          // Bricks, chips, sand, and rings are always available for cash sale (sold regardless of stock)
          const isBulk = catName.includes('sand') || catName.includes('chips') || catName.includes('brick') || catName.includes('ring') ||
                         pName.includes('sand') || pName.includes('chips') || pName.includes('brick') || pName.includes('ring');
          ...
          <div className="space-y-6">
            ...
            <Card className="shadow-lg border-0 bg-white rounded-2xl">
              ...
              <CardContent className="p-4 md:p-6 space-y-4">
                ...
                {productType === 'regular' && (
                  <div className="mb-4">
                    <Label htmlFor="category-select">{t("Category", "श्रेणी")}</Label>
                    <select
                      id="category-select"
                      value={selectedCategoryId}
                      onChange={(e) => {
                        setSelectedCategoryId(e.target.value);
                        setCurrentItem({ ...currentItem, productId: '', name: '', unit: '', price: '', conversionCft: '' as any });
                      }}
                      className="mt-1 h-12 text-base rounded-xl w-full border-gray-300"
                    >
                      <option value="">{t("All Categories", "सभी श्रेणियां")}</option>
                      {Array.from(new Map(products.filter(p => p.category).map(p => [String(p.category.id), p.category])).values()).map((c: any) => (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    >
                      ...
                      {productType === 'regular' ? (
                        products.filter((p: any) => !selectedCategoryId || String(p.category?.id) === selectedCategoryId).map((p: any) => {
