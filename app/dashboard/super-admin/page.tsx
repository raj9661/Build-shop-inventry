...
const DEFAULT_WIDGETS: WidgetConfig[] = [
  ...
  { id: 'todaysProfitOrLoss', title: 'Today\'s Profit/Loss', visible: true, order: 8 },
]
...
function SuperDuperAdminDashboardContent() {
  ...
  const [todaysStats, setTodaysStats] = useState({ revenue: 0, cost: 0, expenses: 0, profit: 0, loaded: false })
        ...
        if (shopsData.success && shopsData.data && shopsData.data.shops) {
          ...
          try {
            ...
            if (analyticsRes.ok) {
              ...
              // Set today's profit/loss stats
              const totalRevenue = Number(analyticsJson?.data?.totalRevenue || 0)
              const totalCostPrice = Number(analyticsJson?.data?.totalCostPrice || 0)
              // We use totalExpenses (shop expenses) + totalEmployeePayments (salary)
              // We DO NOT include totalSupplierPayments here because product cost is already deducted via totalCostPrice (COGS)
              const totalExpenses = Number(analyticsJson?.data?.totalExpenses || 0) + Number(analyticsJson?.data?.totalEmployeePayments || 0)
              
              setTodaysStats({
                revenue: totalRevenue,
                cost: totalCostPrice,
                expenses: totalExpenses,
                profit: totalRevenue - totalCostPrice - totalExpenses,
                loaded: true
              })
            }
          } catch (e) {
              ...
              case 'todaysProfitOrLoss':
                return (
                  ...
                  >
                    ...
                    <CardContent>
                      <div className={`text-2xl font-bold ${todaysStats.profit >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                        {todaysStats.profit >= 0 ? '+' : '-'}₹{Math.abs(todaysStats.profit).toLocaleString()}
                      </div>
                      <p className="text-[10px] sm:text-xs text-muted-foreground mt-1 tracking-tight">
                        S: ₹{todaysStats.revenue.toLocaleString()} | C: ₹{todaysStats.cost.toLocaleString()} | E: ₹{todaysStats.expenses.toLocaleString()}
                      </p>
                    </CardContent>
                  </Card>
                )
